...
//      // all permission check
     ...
     $subMenu =SubMenu::where('url',$routeName)->first(); 
       if (empty($subMenu)) {
           return true;
            ...
            $userpermit= DefaultRoleMenu::where('role_id',$user->role_id)->where('status',1)->where('sub_menu_id',$subMenu->id)->first();
            if (empty($userpermit)) {
                return false;
            }else{
                return true;
  ...
  public static function mainMenuCheck($menu_id){ 
     ...
     return DefaultRoleMenu::where('role_id',$user->role_id)->where('status',1)->exists();  
   }
  //Main menu  check
  public static function subMenuCheck($menu_id,$sub_menu_id){ 
     $user =Auth::guard('admin')->user();
      
     return DefaultRoleMenu::where('role_id',$user->role_id)->where('status',1)->where('sub_menu_id',$sub_menu_id)->exists();  
   }
// // read write delete permission check
