...
//----------------------------------------------
    ...
    public function chcList($value='')
    {
      try {
          ...
          $BlocksMcs= BlocksMc::orderBy('name_e','ASC')->get();   
          $Villages= Village::orderBy('name_e','ASC')->get(); 
          return view('admin.master.chcList.index',compact('Districts','States','BlocksMcs','Villages'));
        } catch (Exception $e) {
            
        }
    }
    public function chcListTable(Request $request)
    { 
      try {
            
          $CHCLists= CHCList::where('districts_id',$request->district_id)->orderBy('name_e','ASC')->get();
          return view('admin.master.chcList.list',compact('CHCLists'));
        } catch (Exception $e) {
            
        }
    }
    public function chcListStore(Request $request,$id=null)
   {   
       $rules=[
            'states' => 'required', 
            'district' => 'required', 
            'code' => 'required|unique:chc_list,code,'.$id, 
            'name' => 'required', 
      ...
      if ($validator->fails()) {
         ...
         $BlocksMc= CHCList::firstOrNew(['id'=>$id]);
         $BlocksMc->states_id=$request->states;
         $BlocksMc->districts_id=$request->district; 
         $BlocksMc->code=$request->code;  
         $BlocksMc->name_e=$request->name; 
